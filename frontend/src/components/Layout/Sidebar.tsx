...
import { LayoutDashboard, Users, FileText, CreditCard, Building2, AlertTriangle, BarChart3, Bell, Settings, LogOut, ChevronLeft, ChevronRight, Shield } from 'lucide-react';
...
export default function Sidebar() {
  ...
  return (
    <aside className={`${collapsed ? 'w-16' : 'w-64'} bg-gray-900 text-white flex flex-col transition-all duration-300 min-h-screen relative flex-shrink-0`}>
      ...
      <button onClick={() => setCollapsed(!collapsed)} className="absolute -right-3 top-6 bg-gray-700 rounded-full p-1 hover:bg-gray-600 transition-colors z-10">
        ...
        {!collapsed ? (
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              <div className="w-8 h-8 bg-blue-500 rounded-full flex items-center justify-center text-xs font-bold">{user?.name?.[0]?.toUpperCase()}</div>
              <div>
            ...
            <button onClick={logout} className="text-gray-400 hover:text-red-400 transition-colors" title="Logout"><LogOut size={16} /></button>
          </div>
        ) : (
          <button onClick={logout} className="text-gray-400 hover:text-red-400 transition-colors" title="Logout"><LogOut size={16} /></button>
        )}
