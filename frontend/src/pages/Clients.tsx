  ...
  return (
    <Layout title="Clients">
      ...
      <div className="card p-0">
        {loading ? <LoadingSpinner /> : clients.length === 0 ? (
          <EmptyState
            ...
            action={<button onClick={() => setShowForm(true)} className="btn-primary">Add Client</button>}
          />
        ) : (
          <>
            <div className="table-container">
              <table className="table">
                <thead>
                  <tr>
                    <th>Client</th><th>National ID</th><th>Phone</th>
                    <th>Email</th><th>Policies</th><th>Actions</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-gray-100">
                  {clients.map(c => (
                    <tr key={c.id}>
                      ...
                      <td>
                        {c.email
                          ? <a href={`mailto:${c.email}`} className="flex items-center gap-1 text-blue-600 hover:underline"><Mail size={12} />{c.email}</a>
                          : '-'}
                      </td>
      ...
      <Modal isOpen={showForm} onClose={() => setShowForm(false)} title={editing ? 'Edit Client' : 'New Client'} size="lg">
        <ClientForm
          initial={editing}
          onSave={() => { setShowForm(false); fetchClients(); }}
